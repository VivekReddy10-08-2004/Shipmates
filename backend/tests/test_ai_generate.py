...
from services.ai_generate import extract_text_from_pdf_bytes, generate_study_drafts, AIGenerateError
...
def _generate_dynamic_mock_json(raw_text: str) -> str:
    # Split raw_text into lines/sentences to make dynamic flashcards & questions
    ...
    for i, s in enumerate(sentences):
        if i >= 10:
            break
        flashcards.append({
            "front": f"What is the key point about {s[:30]}...",
            "back": s
        })
        questions.append({
            "question_text": f"Which statement is true about {s[:30]}?",
            "question_type": "multiple_choice",
            "points": 1,
            "answers": [
                {"answer_text": s, "is_correct": True},
                {"answer_text": f"Incorrect option A for {i}", "is_correct": False},
                {"answer_text": f"Incorrect option B for {i}", "is_correct": False},
                {"answer_text": f"Incorrect option C for {i}", "is_correct": False},
            ]
        })
...
class DynamicMockCompletions:
    def create(self, *args, **kwargs):
        ...
        json_content = _generate_dynamic_mock_json(raw_text)
        
        mock_message  = SimpleNamespace(content=json_content)
...
class DynamicMockClient:
    def __init__(self, *args, **kwargs):
        self.chat = SimpleNamespace(completions=DynamicMockCompletions())


class DynamicMockOpenAIModule:
    OpenAI = DynamicMockClient


@pytest.fixture(autouse=True)
def setup_env_and_mock_openai(monkeypatch):
    monkeypatch.setenv("OPENAI_API_KEY", "sk-fake-key")
    monkeypatch.setenv("AI_GENERATE_PROVIDER", "openai")
    
    with patch.dict(sys.modules, {"openai": DynamicMockOpenAIModule}):
        yield
...
def test_generate_respects_max_items():
    """Optionally output should never exceed 10 flashcards or 10 questions."""
...
def test_openai_path_malformed_json_falls_back(monkeypatch):
    """When OpenAI returns invalid JSON, service must raise AIGenerateError (no local fallback)."""
    monkeypatch.setenv("OPENAI_API_KEY", "sk-fake-key")
    monkeypatch.setenv("AI_GENERATE_PROVIDER", "openai")

    with patch.dict(sys.modules, {"openai": _make_mock_openai("this is not json {{{{")}):
        with pytest.raises(AIGenerateError):
            generate_study_drafts(course_id=7, raw_text=SAMPLE_NOTES_OAI)


def test_openai_path_timeout_falls_back(monkeypatch):
    """When the OpenAI client raises (e.g. timeout), service must raise AIGenerateError (no local fallback)."""
    monkeypatch.setenv("OPENAI_API_KEY", "sk-fake-key")
    monkeypatch.setenv("AI_GENERATE_PROVIDER", "openai")
    ...
    with patch.dict(sys.modules, {"openai": mock_openai}):
        with pytest.raises(AIGenerateError):
            generate_study_drafts(course_id=5, raw_text=SAMPLE_NOTES_OAI)
